...
class OCRPredictor(nn.Module, _OCRPredictor):
    ...
    def forward(
        ...
        txt_filename: str = None,  # new optional argument for saving OCR results
        **kwargs: Any,
    ) -> Document:
        # Dimension check
        ...
        # Write OCR results to the specified file if a filename is provided
        if txt_filename is not None:
            with open(txt_filename, "w") as file:
                for box, token in zip(boxes[0], text_preds[0]):
                    recognized_text = token[0]  # extract the word only
                    # Format the box coordinates as comma-separated values
                    box_str = f"{box[0]}, {box[1]}, {box[2]}, {box[3]}"
                    file.write(f'{box_str} "{recognized_text}"\n')
        return out